...
def _bbox_size_in_mm(width_px, height_px):
    mm_per_pixel_x = float(os.environ.get('PADDLE_MM_PER_PIXEL_X', '1.0'))
    mm_per_pixel_y = float(os.environ.get('PADDLE_MM_PER_PIXEL_Y', '1.0'))
    return width_px * mm_per_pixel_x, height_px * mm_per_pixel_y


def _format_box_text(name, score, width_px=None, height_px=None):
    if width_px is None or height_px is None:
        return "{} {:.2f}".format(name, score)
    width_mm, height_mm = _bbox_size_in_mm(width_px, height_px)
    return "{} {:.2f} {:.2f}x{:.2f}mm".format(name, score, width_mm,
                                             height_mm)
...
def draw_bbox(image, im_id, catid2name, bboxes, threshold):
    ...
    for dt in np.array(bboxes):
        ...
        if len(bbox) == 4:
            ...
            w = max(x1, x2, x3, x4) - xmin
            h = max(y1, y2, y3, y4) - ymin
        else:
            logger.error('the shape of bbox must be [M, 4] or [M, 8]!')

        # draw label
        text = _format_box_text(catid2name[catid], score, w, h)
        tw, th = draw.textsize(text)
        text_top = max(0, ymin - th)
        draw.rectangle(
            [(xmin + 1, text_top), (xmin + tw + 1, text_top + th)], fill=color)
        draw.text((xmin + 1, text_top), text, fill=(255, 255, 255))
...
def draw_segm(image,
    ...
    for dt in np.array(segms):
        ...
        if not draw_box:
            ...
            box_w = x1 - x0
            box_h = y1 - y0
            bbox_text = _format_box_text(catid2name[catid], score, box_w,
                                         box_h)
            t_size = cv2.getTextSize(bbox_text, 0, 0.3, thickness=1)[0]
            text_top = max(0, y0 - t_size[1] - 3)
            cv2.rectangle(img_array, (x0, text_top), (x0 + t_size[0], y0),
                          tuple(color_mask.astype('int32').tolist()), -1)
            cv2.putText(
                img_array,
                bbox_text, (x0, max(t_size[1], y0 - 2)),
