...
class AnchorGeneratorSSD(object):
    ...
    def __call__(self, inputs, image, q_layer0, q_layer1, q_layer2, q_layer3, q_layer4, q_layer5):
        # def __call__(self, inputs, image):
        boxes = []

        # input=inputs[0]
        input = q_layer0(inputs[0])
        box, _ = ops.prior_box(
            input=input,
            image=image,
            min_sizes=_to_list(self.min_sizes[0]),
            max_sizes=_to_list(self.max_sizes[0]),
            aspect_ratios=self.aspect_ratios[0],
            flip=self.flip,
            clip=self.clip,
            steps=[self.steps[0], self.steps[0]],
            offset=self.offset,
        ...
        # input=inputs[1]
        input = q_layer1(inputs[1])
        box, _ = ops.prior_box(
            input=input,
            image=image,
            min_sizes=_to_list(self.min_sizes[1]),
            max_sizes=_to_list(self.max_sizes[1]),
            aspect_ratios=self.aspect_ratios[1],
            flip=self.flip,
            clip=self.clip,
            steps=[self.steps[1], self.steps[1]],
            offset=self.offset,
            min_max_aspect_ratios_order=self.min_max_aspect_ratios_order)
        boxes.append(paddle.reshape(box, [-1, 4]))

        # input=inputs[2]
        input = q_layer2(inputs[2])
        box, _ = ops.prior_box(
            input=input,
            image=image,
            min_sizes=_to_list(self.min_sizes[2]),
            max_sizes=_to_list(self.max_sizes[2]),
            aspect_ratios=self.aspect_ratios[2],
            flip=self.flip,
            clip=self.clip,
            steps=[self.steps[2], self.steps[2]],
            offset=self.offset,
            min_max_aspect_ratios_order=self.min_max_aspect_ratios_order)
        boxes.append(paddle.reshape(box, [-1, 4]))

        input = q_layer3(inputs[3])
        # input=inputs[3]
        box, _ = ops.prior_box(
            input=input,
            image=image,
            min_sizes=_to_list(self.min_sizes[3]),
            max_sizes=_to_list(self.max_sizes[3]),
            aspect_ratios=self.aspect_ratios[3],
            flip=self.flip,
            clip=self.clip,
            steps=[self.steps[3], self.steps[3]],
            offset=self.offset,
            min_max_aspect_ratios_order=self.min_max_aspect_ratios_order)
        boxes.append(paddle.reshape(box, [-1, 4]))

        input = q_layer4(inputs[4])
        # input=inputs[4]
        box, _ = ops.prior_box(
            input=input,
            image=image,
            min_sizes=_to_list(self.min_sizes[4]),
            max_sizes=_to_list(self.max_sizes[4]),
            aspect_ratios=self.aspect_ratios[4],
            flip=self.flip,
            clip=self.clip,
            steps=[self.steps[4], self.steps[4]],
            offset=self.offset,
            min_max_aspect_ratios_order=self.min_max_aspect_ratios_order)
        boxes.append(paddle.reshape(box, [-1, 4]))

        input = q_layer5(inputs[5])
        # input=inputs[5]
        box, _ = ops.prior_box(
            input=input,
            image=image,
            min_sizes=_to_list(self.min_sizes[5]),
            max_sizes=_to_list(self.max_sizes[5]),
            aspect_ratios=self.aspect_ratios[5],
            flip=self.flip,
            clip=self.clip,
            steps=[self.steps[5], self.steps[5]],
            offset=self.offset,
            min_max_aspect_ratios_order=self.min_max_aspect_ratios_order)
        boxes.append(paddle.reshape(box, [-1, 4]))
        return boxes
